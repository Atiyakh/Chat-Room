...
class ConnectionHandler:
    ...
    def communications(self):
        while True:
            if len(self.wrappedRouterConnection.data_sequence.Str) != 0:
                data = self.wrappedRouterConnection.data_sequence['str'].decode(
                    'utf-8')
                function_, connecting_link_id = data.split('|')
                try:
                    encrypted_client_socket = self.createConnection(
                        communication=True, id_=connecting_link_id)
                    try:
                        self.Requests.Communication.__getattribute__(
                            function_)(encrypted_client_socket)
        ...
        try:
            ...
            if request:
                ...
                receiving_thread = Thread(
                    target=wrappedRouterConnection.__recv__)
    ...
    def loadProtocols(self, instance_):
        instance_.RecvProto = self.RecvProto
        instance_.SendProto = self.SendProto
        instance_.RecvSignal = self.RecvSignal
        instance_.SendSignal = self.SendSignal

    def __init__(self, mainclass):
        self.directory = Path(__file__).resolve().parent
        JSON = json.loads(
            open(path.join(self.directory, 'config.json'), 'r').read())
        ...
        try:
            ...
            if encrypted_client_socket:
                ...
                if int(encrypted_client_socket.recv(1)):
                    try:
                        response = self.Requests.__getattribute__(
                            req)(encrypted_client_socket, data)
                        encrypted_client_socket.close()
                        return response
                    except:
                        traceback.print_exc()
                    ...
                    raise ConnectionError(
                        "The request you posted is not defined")
            else:
    ...
    def createConnection(self, communication=False, id_=None):
        try:
            encrypted_client_socket = self.establishSocketActiveConnection(
                request=True)
            ...
            if communication:
                encrypted_client_socket.send(
                    f"{self.entity}|{1}".encode('utf-8'))
                encrypted_client_socket.recv(1)
                encrypted_client_socket.send(str(id_).encode('utf-8'))
                return encrypted_client_socket
            else:
                encrypted_client_socket.send(
                    f"{self.entity}|{0}".encode('utf-8'))
